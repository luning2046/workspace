...
public class LoginActivity extends BaseApiActivity implements OnClickListener, Callback {
    ...
    protected void initView() {
        mUserNameEditText = getViewById(android.R.id.text1);mPasswordEditText = getViewById(android.R.id.text2);
        mRegisterBtn = getViewById(android.R.id.button1);mLoginBtn = getViewById(android.R.id.button2);
        mLoginBtn = getViewById(android.R.id.button2);mVersionTextView = getViewById(R.id.version_code);
    ...
    protected void initData() {
        ...
        mUserNameEditText.setText(email);mPasswordEditText.setText(password);
        mRegisterBtn.setOnClickListener(this); mLoginBtn.setOnClickListener(this);
    ...
    public void onClick(View v) {
        ...
        if (v == mRegisterBtn) {
            ...
            if (TextUtils.isEmpty(username) || TextUtils.isEmpty(password)) { WinToast.toast(LoginActivity.this, R.string.login_erro_is_null);return; }
            if (mDialog != null && !mDialog.isShowing()) mDialog.show();

//=====================发起登录 http请求 (注：非融云SDK接口，是demo接口)==============this 功能注册onCallApiSuccess这个回调========================
    ...
    public void onCallApiSuccess(AbstractHttpRequest request, Object obj) {
        ...
        if (loginHttpRequest == request) {
            if (obj instanceof User) {
                 ...
                 * 通启动了io.rong.imkit.service.RCloudService服务==========connect方法中   启动了lib层消息服务器（RongImClient）并将RongImClient对象放到RCloudContext的单例中===================================================================================================================
                 */
                ...
                Editor editor = DemoContext.getInstance().getSharedPreferences().edit(); editor.putString(INTENT_PASSWORD, mPasswordEditText.getText().toString());
                editor.putString(INTENT_EMAIL, mUserNameEditText.getText().toString());editor.commit();

//====================发起http获取好友信息====================发起获取好友列表的http请求  (注：非融云SDK接口，是demo接口)====================================
getFriendsHttpRequest = DemoContext.getInstance().getDemoApi().getFriends(user.getCookie(), this);//?????

 DemoContext.getInstance().setCurrentUser(user);//==================保存用户信息=============在RongIM中执行完connect方法并成功后，也有用户信息========================
...
//===========================将获得 好友列表放入   DemoContext单例中设置=RongIM.GetFriendsProvider()然后又设置了 RCloudContext.getInstance().setGetFriendsProvider(provider)========================================================================
    ...
    public boolean handleMessage(Message msg) {
        if (msg.what == HANDLER_LOGIN_FAILURE) {
            WinToast.toast(LoginActivity.this, R.string.login_failure); if (mDialog != null) mDialog.dismiss();
        } else if (msg.what == HANDLER_LOGIN_SUCCESS) {
//=====================================登录成功跳转到相应页面=============================================================================================
           

            WinToast.toast(LoginActivity.this, R.string.login_success);if (mDialog != null) mDialog.dismiss();
