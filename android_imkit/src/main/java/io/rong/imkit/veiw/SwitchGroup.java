...
public class SwitchGroup extends ViewGroup {
	...
	public SwitchGroup(Context context, AttributeSet attrs) {
		super(context, attrs);
		TypedArray a = context.obtainStyledAttributes(attrs, ResourceUtils.getStyleablesResourceId(context, "SwitchGroup"));
		mPinnedViewResId = a.getResourceId(ResourceUtils.getStyleableResourceId(context, "SwitchGroup_pinned_view"), 0);
		mOrientation = a.getInt(ResourceUtils.getStyleableResourceId(context, "SwitchGroup_orientation"), 0);
		int itemResId = a.getResourceId(ResourceUtils.getStyleableResourceId(context, "SwitchGroup_pinned_item"), 0);
		int itemArrayResId = a.getResourceId(ResourceUtils.getStyleableResourceId(context, "SwitchGroup_pinned_item_array"), 0);
		a.recycle();
	...
	protected void onAttachedToWindow() {
		...
		if (mPinnedView == null && mPinnedViewResId != 0) {
			...
			mPinnedView = parentView.findViewById(android.R.id.title);
		}
	}
	...
	protected void attachViewToParent(View child, int index, android.view.ViewGroup.LayoutParams params) {
		Log.d(VIEW_LOG_TAG, "attachViewToParent");
