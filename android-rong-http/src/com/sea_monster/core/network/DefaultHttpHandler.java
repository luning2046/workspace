...
public class DefaultHttpHandler implements HttpHandler {
	...
	public DefaultHttpHandler(Context context, ThreadPoolExecutor executor) {
		...
		ConnManagerParams.setMaxTotalConnections(params, Const.SYS.HTTP_MAX_CONN_COUNT);
		ConnPerRouteBean bean = new ConnPerRouteBean(Const.SYS.HTTP_MAX_ROUTE_COUNT);
	...
	protected final HttpParams createHttpParams(int networkType) {
		...
		case ConnectivityManager.TYPE_WIFI:
			HttpConnectionParams.setConnectionTimeout(params, Const.SYS.HTTP_WIFI_TIMEOUT);
			HttpConnectionParams.setSoTimeout(params, Const.SYS.HTTP_WIFI_SOCKET_TIMEOUT);
			break;
		case ConnectivityManager.TYPE_MOBILE:
			HttpConnectionParams.setConnectionTimeout(params, Const.SYS.HTTP_GPRS_TIMEOUT);
			HttpConnectionParams.setSoTimeout(params, Const.SYS.HTTP_GPRS_SOCKET_TIMEOUT);
			break;
	...
	public <T> int executeRequest(AbstractHttpRequest<T> request) {
		...
		if (executor.getQueue().size() >= Const.SYS.WORK_QUEUE_MAX_COUNT) {
	...
	public class PriorityRequestRunnable<T> extends PriorityRunnable {
		...
		public void run() {
			...
			if (request.getResStream() != null && currentNetworkType == ConnectivityManager.TYPE_MOBILE) {
				uriRequest.getParams().setIntParameter(CoreConnectionPNames.SO_TIMEOUT, Const.SYS.HTTP_GPRS_RES_SOCKET_TIMEOUT);
			}
						...
						if (length > 20000) {
							if (adapter instanceof BasicPooledConnAdapter) {
								...
								if (currentNetworkType == ConnectivityManager.TYPE_WIFI) {
									timeout = (int) (length / Const.SYS.HTTP_WIFI_SPEED);
									timeout = timeout > Const.SYS.HTTP_WIFI_SOCKET_TIMEOUT ? timeout : Const.SYS.HTTP_WIFI_SOCKET_TIMEOUT;

								} else {
									timeout = (int) (length / Const.SYS.HTTP_GPRS_SPEED);
									timeout = timeout > Const.SYS.HTTP_GPRS_SOCKET_TIMEOUT ? timeout : Const.SYS.HTTP_GPRS_SOCKET_TIMEOUT;
								}
	...
	public <T> T executeRequestSync(AbstractHttpRequest<T> request) throws BaseException {
		...
		try {
			...
			if (response.getStatusLine().getStatusCode() == 200) {
				...
				Log.d("DefaultHttpHandler", "--------statue---1------->" + statue);
				...
				Log.d("DefaultHttpHandler", "--------statue---2------->" + statue);
